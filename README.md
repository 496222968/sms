...
```shell script
php bin/hyperf.php vendor:publish dwc/sms
```
...
如果你希望你的短信类始终使用队列，您可以给短信消息类实现 `Dwc\Contract\ShouldQueue` 接口，现在即使你调用了 `send` 方法，短信依旧使用队列的方式发送。另外，如果需要将短信推送到指定队列，可以设置在短信消息类中设置 `queue` 属性。

```php
use Dwc\Contract\ShouldQueue;
use Dwc\Sms\Smsable;
